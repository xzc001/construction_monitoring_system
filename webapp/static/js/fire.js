...
const STATE_META = {
  no_fire:    { cls: "s-no_fire",    text: "未见烟火",        desc: "画面内未检测到烟雾或明火" },
  fire_smoke: { cls: "s-fire_smoke", text: "预警 · 烟雾/明火", desc: "检测到烟雾或明火 —— 火灾隐患,已触发告警!" },
};

// ---------- 加载样本(ready 模块通常无内置样本) ----------
(async function init() {
  let samples = [];
  try { samples = await api("/api/samples?module=fire"); } catch (e) { samples = []; }
...
async function loadResult(run) {
  ...
  renderTimeline(r);
  renderAlerts(r);
  setStatus(r.segments.length ? r.segments[0].state : "no_fire", 0);
...
function setStatus(state) {
  const m = STATE_META[state] || STATE_META.no_fire;
...
function currentSegmentState(t) {
  if (!RESULT) return "no_fire";
  for (const s of RESULT.segments) if (t >= s.start_s && t < s.end_s) return s.state;
  return RESULT.segments.length ? RESULT.segments[RESULT.segments.length - 1].state : "no_fire";
}
...
runBtn.onclick = async () => {
  ...
  fd.append("fire_conf", document.getElementById("conf").value.trim() || "0.40");
  ...
  let job;
  try { job = await api("/api/fire/analyze", { method: "POST", body: fd }); }
