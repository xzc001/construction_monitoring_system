...
const STATE_META = {
  compliant: { cls: "s-compliant", text: "合规 · 已戴安全帽", desc: "画面内人员均已佩戴安全帽,符合规范" },
  violation: { cls: "s-violation", text: "违规 · 未戴安全帽", desc: "检测到作业人员未佩戴安全帽 —— 头部受伤风险,已触发告警!" },
};

// ---------- 加载样本 ----------
(async function init() {
  let samples = [];
  try { samples = await api("/api/samples?module=helmet"); } catch (e) { return; }
...
async function loadResult(run) {
  ...
  renderTimeline(r);
  renderAlerts(r);
  setStatus(r.segments.length ? r.segments[0].state : "compliant", 0);
...
function setStatus(state, t) {
  const m = STATE_META[state] || STATE_META.compliant;
...
function currentSegmentState(t) {
  if (!RESULT) return "compliant";
  for (const s of RESULT.segments) {
    if (t >= s.start_s && t < s.end_s) return s.state;
  }
  return RESULT.segments.length ? RESULT.segments[RESULT.segments.length - 1].state : "compliant";
}
...
runBtn.onclick = async () => {
  ...
  fd.append("helmet_conf", document.getElementById("conf").value.trim() || "0.40");
  ...
  let job;
  try { job = await api("/api/helmet/analyze", { method: "POST", body: fd }); }
