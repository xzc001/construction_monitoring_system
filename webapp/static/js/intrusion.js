...
const STATE_META = {
  clear:     { cls: "s-clear",     text: "安全 · 无人闯入",     desc: "划定区域内无人,现场处于安全状态" },
  warning:   { cls: "s-warning",   text: "警戒 · 接近危险区",   desc: "有人员进入警戒区 —— 仅作提示,未达告警条件" },
  intrusion: { cls: "s-intrusion", text: "闯入 · 危险区域有人", desc: "人员闯入禁区并持续停留 —— 存在人身安全风险,已触发告警!" },
};

// ---------- 加载样本 ----------
(async function init() {
  let samples = [];
  try { samples = await api("/api/samples?module=intrusion"); } catch (e) { return; }
...
async function loadResult(run) {
  ...
  renderTimeline(r);
  renderAlerts(r);
  setStatus(r.segments.length ? r.segments[0].state : "clear", 0);
...
function setStatus(state, t) {
  const m = STATE_META[state] || STATE_META.clear;
...
function currentSegmentState(t) {
  ...
  return RESULT.segments.length ? RESULT.segments[RESULT.segments.length - 1].state : "clear";
}
...
runBtn.onclick = async () => {
  ...
  let job;
  try { job = await api("/api/intrusion/analyze", { method: "POST", body: fd }); }
