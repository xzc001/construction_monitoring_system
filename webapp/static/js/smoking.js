...
const STATE_META = {
  no_smoking: { cls: "s-no_smoking", text: "未见吸烟", desc: "画面内未检测到吸烟行为,符合禁烟规范" },
  smoking:    { cls: "s-smoking",    text: "违规 · 检测到吸烟", desc: "检测到人员吸烟 —— 禁烟区火灾隐患,已触发告警!" },
};

// ---------- 加载样本 ----------
(async function init() {
  let samples = [];
  try { samples = await api("/api/samples?module=smoking"); } catch (e) { return; }
...
async function loadResult(run) {
  ...
  renderTimeline(r);
  renderAlerts(r);
  setStatus(r.segments.length ? r.segments[0].state : "no_smoking", 0);
...
function setStatus(state, t) {
  const m = STATE_META[state] || STATE_META.no_smoking;
...
function currentSegmentState(t) {
  if (!RESULT) return "no_smoking";
  for (const s of RESULT.segments) {
    if (t >= s.start_s && t < s.end_s) return s.state;
  }
  return RESULT.segments.length ? RESULT.segments[RESULT.segments.length - 1].state : "no_smoking";
}
...
runBtn.onclick = async () => {
  ...
  fd.append("smoking_conf", document.getElementById("conf").value.trim() || "0.50");
  ...
  let job;
  try { job = await api("/api/smoking/analyze", { method: "POST", body: fd }); }
